...
import { ResourceFactory } from '../resource/resource';
...
export class Model extends ElementBase {
    ...
    */
    controllerAttached: ModelEvent<IController> = new ModelEvent<IController>();

    /**
      Controll detached event
     */
    controllerDetached: ModelEvent<IController> = new ModelEvent<IController>();
    ...
    */
    constructor() {
        ...
        /*
        this.addTextResource = this.addTextResource.bind(this);
        this.addTextResourceUri = this.addTextResourceUri.bind(this);
        this.addBitmapResource = this.addBitmapResource.bind(this);
        this.addModelResource = this.addModelResource.bind(this);
        */
    ...
    */
    static parse(json: string) {
        ...
        model.parse(o);
        if (o.resources) {
            o.resources.forEach(function(value: Resource) {
        ...
        if (o.elements) {
            o.elements.forEach(function(value: Resource) {
    ...
    */
    static load(basePath: string, uri: string, callback: (model: Model) => void) {
        ...
        Utility.getRemoteText(modelFilePath, function(json) {
            if (json) {
    ...
    */
    listen(rm: ResourceManager, state: ResourceState): void {
    ...
    */
    setModelPath(path: string, resourceFolder?: string) {
        this.modelPath = path;
    ...
    */
    ...
    /**
      ...
      @returns New element index
    */
    addBottom(el: ElementBase): number {
        /*
        if (this.elements.indexOf(el) !== -1) {
            throw new EliseException("Element already exists in collection in Model.add.");
        }*/
    ...
    */
    assignCanvas(canvas: HTMLCanvasElement, scale?: number) {
        this.canvas = canvas;
        let s = 1;
        if (scale && scale > 0) {
            s = scale;
    ...
    /* Removed to break circular dependencies
    addTextResource(key: string, text: string, locale?: string): TextResource {
        let tr: TextResource;
        if(arguments.length === 2) {
            tr = TextResource.createFromText(key, text);
        }
        else {
            tr = TextResource.createFromText(key, text, locale);
        }
        this.resourceManager.merge(tr);
        return tr;
    }

    addTextResourceUri(key: string, uri: string, locale?: string): TextResource {
        let tr: TextResource;
        if(arguments.length === 2) {
            tr = TextResource.createFromUri(key, uri);
        }
        else {
            tr = TextResource.createFromUri(key, uri, locale);
        }
        this.resourceManager.merge(tr);
        return tr;
    }

    addBitmapResource(key: string, uriOrImage: string | HTMLImageElement, locale?: string): BitmapResource {
        let br: BitmapResource;
        if(arguments.length === 2) {
            br = BitmapResource.create(key, uriOrImage);
        }
        else {
            br = BitmapResource.create(key, uriOrImage, locale);
        }
        this.resourceManager.merge(br);
        return br;
    }

    addModelResource(key: string, uriOrModel: string | Model, locale?: string): ModelResource {
        let mr: ModelResource = null;
        if(arguments.length === 2) {
            mr = ModelResource.create(key, uriOrModel);
        }
        else {
            mr = ModelResource.create(key, uriOrModel, locale);
        }
        this.resourceManager.merge(mr);
        return mr;
    }
    */
    ...
    */
    getResourceKeyReferenceCounts(localeId: string) {
        const keys = {};
        ...
        for (let i = 0; i < ml; i++) {
            ...
            if (element.getResourceKeys) {
                ...
                for (let j = 0; j < elementKeys.length; j++) {
                    if (!keys[elementKeys[j]]) {
                        keys[elementKeys[j]] = 1;
                    }
                    else {
                        keys[elementKeys[j]] += 1;
                    }
    ...
    */
    prepareResources(localeId: string, callback: (result: boolean) => void) {
        const rm = this.resourceManager;
        const model = this;
    ...
    */
    setRenderTransform(c: CanvasRenderingContext2D, t: string, origin: Point) {
    ...
    */
    firstActiveElementAt(c: CanvasRenderingContext2D, tx: number, ty: number) {
    ...
    */
    ...
    /**
      ...
      @param id - Element ID
      @returns First element with given ID or null if not found
    */
    ...
    /**
      ...
      @param scale - Rendering scale. Default is 1.
    */
    renderToContext(c: CanvasRenderingContext2D, scale?: number) {
    ...
    */
    strokeForElement(el: ElementBase): string {
        let compare = el;
        while (compare) {
            if (compare.stroke) {
